...
class Sequence(Scene):
    def construct(self):
        ...
        self.play(AnimationGroup(anim_up, anim_center, anim_down, lag_ratio=1.0))
        self.wait(1)
        self.clear()
        ...
        anim_center = self._circle(0, duration=0.5)
        anim_down = self._circle(DOWN, duration=2)
        self.play(AnimationGroup(anim_up, anim_center, anim_down, lag_ratio=0.0))
        self.wait(1)
        self.clear()
    ...
    def _circle(pos, duration):
        ...
        circle = Circle(
            radius=1, color=color_hex, fill_color=color_hex, fill_opacity=0.5
        )
        ...
        anim = group.animate(
            rate_func=rate_functions.ease_out_bounce, run_time=duration
        ).move_to(RIGHT + pos * 2)
